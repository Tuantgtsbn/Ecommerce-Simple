...
const addAddress = createAsyncThunk('ShoppingAddAddress', async (data, { rejectWithValue }) => {
    try {
...
const editAddress = createAsyncThunk(
    ...
    async ({ addressId, formData }, { rejectWithValue }) => {
        try {
            const response = await axiosClient.put(`/api/shop/address/${addressId}`, formData, {
                withCredentials: true
            });
            return response.data;
        } catch (error) {
...
const deleteAddress = createAsyncThunk(
    ...
    async (addressId, { rejectWithValue }) => {
        try {
            const response = await axiosClient.delete(`/api/shop/address/${addressId}`, {
                withCredentials: true
            });
            return response.data;
        } catch (error) {
...
export const { setAddressEdited, setAddressChoosen } = ShoppingAddressSlice.actions;
