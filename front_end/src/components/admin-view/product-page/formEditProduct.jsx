...
function EditFormProduct({ initialFormData, setIsOpenAddProductDialog }) {
    ...
    const checkValid = editProductFormElements().every((element) => formData[element.name] !== '');
    const { toast } = useToast();
    // const [linksImagesProductDelete, setLinksImagesProductDelete] = useState([]);
    const [linksThumbnailProductDelete, setLinksThumbnailProductDelete] = useState(null);
        ...
        <>
            ...
            <div className='relative bg-gray-300'>
                ...
                <button className='absolute top-[10px] right-1 rounded-full bg-gray-300 hover:bg-gray-500'>
                    <X
                        size={16}
                        onClick={() => setLinksThumbnailProductDelete(initialFormData.thumbnail)}
                    />
                </button>
            </div>
