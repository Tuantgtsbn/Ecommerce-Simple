...
function PostByCategoryPage() {
  ...
  useEffect(() => {
    ...
    async function fetchFeaturedPosts(id) {
      if (!id) return;
      try {
    ...
    async function fetchMostViewPosts(id) {
      if (!id) return;
      try {
        const response = await axiosClient.post("/api/shop/post", {
          limit: 5,
    ...
    async function fetchNewestPosts(id) {
      if (!id) return;
      try {
        const response = await axiosClient.post("/api/shop/post", {
          limit: 5,
    ...
    async function fetchData() {
      setLoading(true);
      await fetchBlogCategory();
      await fetchCategories();
      await fetchFeaturedPosts(blogCategory?._id);
      await fetchMostViewPosts(blogCategory?._id);
      await fetchNewestPosts(blogCategory?._id);
      setLoading(false);
    }
    fetchData();
  }, [name, blogCategory?._id]);
  ...
  useEffect(() => {
    window.document.title = blogCategory?.name || "Loading...";
  }, [blogCategory?.name]);
  return loading ? (
    ...
    <>
      <div className="bg-[#EDF1F5]  px-[28px] h-[340px] flex flex-col py-[20px]">
        ...
        <div className="container mx-auto">
          <p className="text-[44px] uppercase font-bold mb-[12px]">
            Ecommerce Blog
          </p>
      ...
      <div className="container mx-auto mt-[50px] lg:mt-[80px]">
        {/* List post */}
        <p className="font-bold text-[32px] mb-[24px]">
      ...
      <div className="container mx-auto mt-[50px] lg:mt-[80px]">
        ...
        <div className="flex justify-between">
          ...
          {!isMobile && (
            <p className="text-[24px] text-red-500">Xem thêm &gt;</p>
          )}
        </div>
